/*
P53 - Days Until End Of Year

Write a program to read a date,
then calculate how many days remain until the end of the year.
...
- The number of days remaining until the end of the year.
...
{
	return GetDaysFromBeginningofYear(Date.Year, Date.Month, Date.Day);
}

short DaysUntilEndOfYear(stDate Date)
{
	
	return  GetDaysInYear(Date.Year) - GetDaysFromBeginningofYear(Date);
}
