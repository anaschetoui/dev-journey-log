/*
P54 - Calculate Actual Vacation Days

Write a program to read a vacation start date
and a vacation end date.

Create a function to calculate the actual vacation
...
A vacation day is counted only if it is a business day.
...
int main()
{
	...
	cout << "\nVacation From: " <<GetDayName(GetDayOrder(StartDate))<<", " << GetDate(StartDate) << endl;
	cout << "\nVacation To  : " << GetDayName(GetDayOrder(EndDate)) << ", " << GetDate(EndDate) << endl;

	cout << "\nActual Vacation Days is: " << CalculateActualVacation(StartDate, EndDate) << endl;;
