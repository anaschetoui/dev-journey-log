...
std::string getDateString(short NumberOfDaysInYear,short Year) 
{
	short YearInDate = Year;
	short MonthInDate = 1;
	short DayInDate = 0;
	short DaysInMonth = 0;
	short DaysLeft = NumberOfDaysInYear;
	

	while (true)
	{
		DaysInMonth = NumberOfDayInMonth(MonthInDate, Year);
	...
	return std::to_string(DayInDate) + "\\" + std::to_string(MonthInDate) + "\\" + std::to_string(Year);
}

int main()
{
	short Year = ReadYear(), Month = ReadMonth(), Day = ReadDay(Year, Month);
	short TotalDaysInYear = HowManyDaysInThisYear(Day, Month, Year);

	std::cout << "\nNumber of Days from beginning of Year " << Year << " is " << TotalDaysInYear << " Day(s).\n";
	std::cout << "\nDate For " << TotalDaysInYear << " is : " << getDateString(TotalDaysInYear, Year) << std::endl;
	return 0;
}